...
- Public runtime restored after commit `898b96c`: direct external HTTP probes returned 200 from the API with buildRevision `898b96c` and from the Pages entry containing the updated origin `https://since-document-isp-armor.trycloudflare.com`. Entry: `https://shuhongguo56-crypto.github.io/ai-ppt-agent/live/`. This confirms reachability/version only, not full customer generation or permanent uptime. Public backend still depends on this machine and a quick tunnel.
...
- Verification: 368 Python tests passed, including final-artifact truncation detection, semantic bilingual title condensation, outline-rehydrated repair behavior, and the full render/quality suite.
