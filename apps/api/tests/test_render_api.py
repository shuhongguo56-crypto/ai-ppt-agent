...
def test_visual_asset_resolution_recovers_real_asset_from_prior_repair_version(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setattr(render_service.time, "sleep", lambda _seconds: None)
    project_root = tmp_path / "project-repair"
    old_assets = project_root / "slide-deck-v1" / "assets"
    current_render = project_root / "slide-deck-v2"
    old_assets.mkdir(parents=True)
    old_image = old_assets / "slide-1-ai.jpg"
    old_image.write_bytes(b"\xff\xd8\xff\xe0real-prior-version-image")
    (old_assets / "slide-1-asset.json").write_text(
        __import__("json").dumps(
            {
                "slide": 1,
                "fileName": old_image.name,
                "mimeType": "image/jpeg",
                "sourceType": "free_ai_fallback",
                "alt": "Prior page-specific image",
                "query": "Luckin Coffee growth quality",
                "imageType": "product_showcase",
                "purpose": "Explain the closing judgment",
                "prompt": "award-winning closing visual with a single focal subject",
                "providerChain": ["Pollinations FLUX API"],
                "attribution": "Generated by a free image provider",
            }
        ),
        encoding="utf-8",
    )
    ...
    slide = SimpleNamespace(
        slide_index=1,
        title="Growth quality matters",
        visual_intent="Explain the closing judgment",
        design_plan=SimpleNamespace(
            asset_role="metaphor",
            image_treatment="full_bleed",
            composition_archetype="manifesto_close",
        ),
    )
    image_plan = SimpleNamespace(
        slide=1,
        search_query="Luckin Coffee growth quality",
        image_type="product_showcase",
        purpose="Explain the closing judgment",
        prompt="award-winning closing visual with a single focal subject",
        provider_chain=["Pollinations FLUX API"],
    ...
    assert assets[1].source_type == "free_ai_fallback"
